...
<div class="container">
<form action= "/submit_answer/{{$timer->id}}/{{$questionaire[0]->language}}" method="post" name="quiz" id="quiz_form" onsubmit="myFunction()">	
@csrf									
		<ol class="breadcrumb">
		...
		<h2> {{$users->language ?? ''}} 's {{$users->title ?? ''}}  MCQ Questions </h2>
			 <div class="row ">
			 <div class="col-lg-12 col-md-12 col-12">
			 @foreach($users->questions as $key=> $question)
			
					<p class="question" ><strong>Q{{$key +1 }}.{{$question->question ?? ''}}</strong></p>	
					@foreach($question->answers as $answer)
					<label for="answer{{$answer->id}}">
					<li class="list-group-item">
					<input type="radio" name="response[{{$key}}][answer_id]" value="{{$answer->id}}" id="answer{{$answer->id ?? ''}}"> 
					{{$answer->answer ?? ''}}
